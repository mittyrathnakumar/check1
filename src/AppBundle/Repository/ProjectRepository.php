		...
		if(!empty($ProjectID)){
			while($row = oci_fetch_array($queryParse, OCI_ASSOC+OCI_RETURN_NULLS)){
				...
				*/
				...
				$projectDetail['STAutomationApplicable'] = $row['ST_AUTOMATION_APPLICABLE'];				
			...
			while ($row = oci_fetch_array($queryParse, OCI_ASSOC+OCI_RETURN_NULLS)) {
				...
				$projectDetail->setDomain($row['DOMAIN']);
				$projectDetail->setScope($row['SCOPE']);				
				$projectDetail->setSTAutomationApplicable($row['ST_AUTOMATION_APPLICABLE']);
				$projectDetail->setUatApplicable($row['UAT_APPLICABLE']);				
		...
		if(empty($postData['ProjectID'])){
			...
			if($row['COUNT'] == 0){			
				...
				$query = "INSERT INTO KPI_PROJECTS (PROJECTID, PROJECTNAME, QC_PROJECTNAME, QC_TABLENAME, POC, DOMAIN,
					...
					NO_OF_ST_AUTOMATED_TEST_CASES,TOTAL_NO_OF_ST_TEST_CASES,ST_AUTOMATION, ST_AUTOMATION_APPLICABLE,
					ADDED_BY, ADDED_ON)
					VALUES (PROJECTS_PROJECTID_SEQ.nextval, '".$objrv->rv('ProjectName')."', '".$objrv->rv('QCProjectName')."',
							...
							'".$stAutomation."', '".$objrv->rv('stAutomationApplicable')."', ".$userID.", '".date("d/M/Y h:i:s A")."')";
			...
			if($row['COUNT'] == 0){
				...
				$query = "UPDATE KPI_PROJECTS SET  					
						...
						ST_AUTOMATION_APPLICABLE = '".$objrv->rv('stAutomationApplicable')."',
						EDITED_BY = ".$userID.",
